...
export async function POST(req: Request) {
 try {
  ...
  try {
   ...
   if (payload) {
    const cache = await redis.get(settings + 'backdrops')
    if (cache) {
     return Response.json(JSON.parse(cache))
    } else {
     let raw = await prisma.backdrops.findUnique({
      where: {
       id: parseInt(settings),
      },
     })
     if (raw) {
      await redis.set(settings + 'backdrops', JSON.stringify(raw), 'EX', 60 * 60 * 2)
